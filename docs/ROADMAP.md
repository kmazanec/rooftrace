...
| **Pipeline contract** | `shared/pipeline_schema.json` (delivered by F-02) | Every Rails↔sidecar call validates request and response against this schema. Schema changes are PRs that bump the contract version and update both clients. |
...
| **Deploy story** | [ADR-011](./adrs/ADR-011-deploy-kamal-do-droplet.md) (delivered by F-01) | All services run as containers; Kamal is the deploy tool; `ops/deploy.yml` is the production source of truth. |
