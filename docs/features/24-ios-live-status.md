...
**ID:** F-24 · **Roadmap piece:** F-24 · **Status:** Done
...
- [x] `StatusPollViewModel` tests (fake clock + FakeAPIClient): `processing→processing→ready` advances then stops; cancellation stops; transient throw→backoff escalates; good poll→resets; `failed` surfaces reason + stops; 401→`handleUnauthorized`.
- [x] Implement the poll loop (interval state, backoff, terminal-stop, cancellation checks).
- [x] Map `Stage.allCases`→timeline rows + unit test each status→correct timeline.
- [x] Build `SegmentedProgress`/`ProgressDots` + the vertical timeline (reduced-motion pulse).
- [x] Build `JobStatusView` (timeline + meter + ready actions + failed recoverable block).
- [x] Wire ready→`.report`, scan→`.capture(handoff)`, failed→try-again/back; 401→re-auth.
- [x] Confirm `.task(id:)` lifetime + disappear teardown.
